...
euler_flux(const Tensor<1, dim + 2, Number> &q) {
    const Tensor<1, dim, Number> velocity = euler_velocity<dim>(q);
    const Number pressure = euler_pressure<dim>(q);
...
inline DEAL_II_ALWAYS_INLINE Tensor<1, dim + 2, Number> euler_numerical_flux(
    ...
    const auto pressure_in = euler_pressure<dim>(q_in);
    const auto pressure_out = euler_pressure<dim>(q_out);

    const auto flux_in = euler_flux<dim>(q_in);
    const auto flux_out = euler_flux<dim>(q_out);
...
template <int dim, typename Number>
VectorizedArray<Number> evaluate_function(
...
Tensor<1, n_components, VectorizedArray<Number>> evaluate_function(
    const Function<dim> &function,
    const Point<dim, VectorizedArray<Number>> &p_vectorized) {
...
void EulerOperator<dim, degree, n_points_1d>::set_subsonic_outflow_boundary(
    ...
    AssertThrow(
        inflow_boundaries.find(boundary_id) == inflow_boundaries.end() &&
            wall_boundaries.find(boundary_id) == wall_boundaries.end(),
        ExcMessage("You already set the boundary with id " +
                   std::to_string(static_cast<int>(boundary_id)) +
                   " to another type of boundary before now setting " +
...
void EulerOperator<dim, degree, n_points_1d>::local_apply_cell(
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second;
        ...
        const auto forcing_eval_flags = (body_force.get() != nullptr)
                                            ? EvaluationFlags::values
                                            : EvaluationFlags::nothing;
        phi.integrate_scatter(forcing_eval_flags | EvaluationFlags::gradients,
                              dst);
    }
}
...
void EulerOperator<dim, degree, n_points_1d>::local_apply_face(
    ...
    for (unsigned int face = face_range.first; face < face_range.second;
         ++face) {
        phi_out.reinit(face);
        phi_out.gather_evaluate(src, EvaluationFlags::values);

        phi_in.reinit(face);
        phi_in.gather_evaluate(src, EvaluationFlags::values);

        for (const unsigned int q : phi_in.quadrature_point_indices()) {
            const auto numerical_flux = euler_numerical_flux<dim>(
                phi_in.get_value(q), phi_out.get_value(q),
                phi_in.normal_vector(q));
            phi_in.submit_value(-numerical_flux, q);
            phi_out.submit_value(numerical_flux, q);
        }

        phi_out.integrate_scatter(EvaluationFlags::values, dst);
        phi_in.integrate_scatter(EvaluationFlags::values, dst);
    }
}
...
void EulerOperator<dim, degree, n_points_1d>::local_apply_boundary_face(
    ...
    for (unsigned int face = face_range.first; face < face_range.second;
        ...
        for (const unsigned int q : phi.quadrature_point_indices()) {
            const auto w_in = phi.get_value(q);
            const auto normal = phi.normal_vector(q);

            auto rho_u_dot_n = w_in[1] * normal[0];
            for (unsigned int d = 1; d < dim; d++) {
                rho_u_dot_n += w_in[d + 1] * normal[d];
            }

            Tensor<1, dim + 2, VectorizedArray<Number>> w_out;
            ...
            if (wall_boundaries.find(boundary_id) != wall_boundaries.end()) {
                ...
                // Reverse momentum at wall boundaries
                for (unsigned int d = 0; d < dim; d++) {
                    w_out[d + 1] = w_in[d + 1] - 2. * rho_u_dot_n * normal[d];
                }
                w_out[dim + 1] = w_in[dim + 1];
            }
            // Inflow boundary
            else if (inflow_boundaries.find(boundary_id) !=
                     inflow_boundaries.end()) {
                w_out = evaluate_function(
            ...
            // Subsonic outflow boundary
                ...
                AssertThrow(
                    false,
                    ExcMessage("Unknown boundary id, did you set a boundary "
                               "condition for this part of the boundary?"));
            }

            auto flux = euler_numerical_flux(w_in, w_out, normal);
            if (at_outflow) {
...
void EulerOperator<dim, degree, n_points_1d>::apply(
    ...
    auto data = this->data();
    {
        ...
        data.loop(&EulerOperator::local_apply_cell,
                  ...
                  &EulerOperator::local_apply_boundary_face, this, dst, src,
...

    {
        ...
        data.cell_loop(&EulerOperator::local_apply_inverse_mass_matrix, this,
                       dst, dst);
    }
}
...
void EulerProblem<dim>::Postprocessor::evaluate_vector_field(
    ...
    for (unsigned int p = 0; p < n_evaluation_points; ++p) {
        ...
        if (do_schlieren_plot == true)
            computed_quantities[p](dim + 2) = inputs.solution_gradients[p][0] *
...
std::vector<std::string> EulerProblem<dim>::Postprocessor::get_names() const {
    ...
    for (unsigned int d = 0; d < dim; ++d) names.emplace_back("velocity");
    names.emplace_back("pressure");
    names.emplace_back("speed_of_sound");

    if (do_schlieren_plot == true) names.emplace_back("schlieren_plot");
...
void EulerProblem<dim>::make_grid_and_dofs() {
    switch (testcase) {
        case 0: {
            ...
            for (unsigned int d = 1; d < dim; ++d) lower_left[d] = -5;

            Point<dim> upper_right;
            upper_right[0] = 10;
            for (unsigned int d = 1; d < dim; ++d) upper_right[d] = 5;
...
void EulerProblem<dim>::output_results(const unsigned int result_number) {
...

    {
        ...
        if (testcase == 0 && dim == 2) {
            ...
            data_out.add_data_vector(dof_handler, reference, names,
                                     interpretation);
        }
...
void EulerProblem<dim>::run() {
    ...
    while (time < final_time - 1e-12) {
...

        {
            ...
            integrator.perform_time_step(euler_operator, time, time_step,
                                         solution, rk_register_1,
                                         rk_register_2);
        }
