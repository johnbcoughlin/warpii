...
class Species {
   public:
       Species(std::string name, double charge, double mass, EulerBCMap<dim> bc_map) 
           : name(name), charge(charge), mass(mass), bc_map(bc_map) {}

    static void declare_parameters(ParameterHandler &prm, unsigned int n_boundaries);

    static Species<dim> create_from_parameters(ParameterHandler &prm);
...
template <int dim>
void Species<dim>::declare_parameters(ParameterHandler &prm, unsigned int n_boundaries) {
    prm.declare_entry("name", "neutral", Patterns::Selection("neutral|ion|electron"));
...
template <int dim>
Species<dim> Species<dim>::create_from_parameters(ParameterHandler &prm) {
    ...
    prm.enter_subsection("BoundaryConditions");
    {
        for (unsigned int i = 0; i < MAX_BOUNDARY_CONDITIONS; i++) {
            ...
            if (bc_type == "Wall") {
                ...
                ss << i << "_Inflow";
                prm.enter_subsection(ss.str());
                std::unique_ptr<Functions::ParsedFunction<dim>> inflow_func = std::make_unique<Functions::ParsedFunction<dim>>(dim+2);
    ...
    return Species<dim>(name, charge, mass, bc_map);
}
