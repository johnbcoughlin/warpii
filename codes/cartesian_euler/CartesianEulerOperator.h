...
void CartesianEulerOperator<dim, degree, n_points_1d>::
    local_apply_boundary_face(
        ...
        const LinearAlgebra::distributed::Vector<Number> &src,
        const std::pair<unsigned int, unsigned int> &face_range) const {
    FEFaceEvaluation<dim, degree, n_points_1d, dim + 2, Number, VA> phi(data, true);
    ...
    for (unsigned int face = face_range.first; face < face_range.second;
        ...
        for (const unsigned int q : phi.quadrature_point_indices()) {
            ...
            if (_bc_map.is_inflow(boundary_id)) {
                w_p = evaluate_function<dim, Number, VA>(*_bc_map.get_inflow(boundary_id),
            ...
            if (at_outflow) {
                for (unsigned int v = 0; v < VA::size();
                     ++v) {
...
void CartesianEulerOperator<dim, degree, n_points_1d>::
    ...
    MatrixFreeOperators::CellwiseInverseMassMatrix<dim, degree, dim + 2, Number, VA>
        inverse(phi);
...
local_apply_positivity_limiter(
    ...
    // This should be constructed from the quadrature rule used for the positivity-preserving step.
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second;
         ...
         */
        auto num_rho = std::max(rho_bar - 
                (1e-12 * std::max(rho_bar, VA(1.0))), 
                VA(0.0));
        ...
        for (unsigned int v = 0; v < VA::size(); ++v) {
            ...
            if (theta_rho_v != 1.0 || theta_E_v != 1.0) {
                //std::cout << "theta_rho: " << theta_rho_v << "; theta_E: " << theta_E_v << std::endl;
                //std::cout << "theta_rho: " << theta_rho_v << "; denom_rho: " << denom_rho[v] << std::endl;
                //std::cout << "min_rho: " << rho_min[v] << "; rho_bar: " << cell_avg[0][v] << std::endl;
            }
        }
        ...
        for (const unsigned int q : phi.quadrature_point_indices()) {
            ...
            for (unsigned int vec_i = 0; vec_i < VA::size(); ++vec_i) {
                //AssertThrow(v[dim+2][vec_i] > 1e-12, ExcMessage("Submitting negative density to quad point"));
                ...
                AssertThrow(rho[vec_i] > 1e-12, ExcMessage("Submitting negative density to quad point"));
                AssertThrow(pressure[vec_i] > 1e-12, ExcMessage("Submitting negative pressure to quad point"));
...
void CartesianEulerOperator<dim, degree, n_points_1d>::
        ...
        data.cell_loop(
            ...
            [&](const unsigned int start_range, const unsigned int end_range) {
                ...
                for (unsigned int i = start_range; i < end_range; ++i) {
                    ...
                    sol_before_limiting.local_element(i) = beta * dst_i + alpha * (u_i + dt * dudt_i);
                }
            });
    }

    {
        ...
        data.cell_loop(
                &CartesianEulerOperator::local_apply_positivity_limiter, this,
    ...
    for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell) {
        ...
        for (const unsigned int q : phi.quadrature_point_indices())
            phi.submit_dof_value(
                evaluate_function<dim, Number, VA>(function, phi.quadrature_point(q)), q);
