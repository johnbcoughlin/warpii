...
class RadialEulerOperator {
   public:
    RadialEulerOperator(TimerOutput &timer, double gamma) : timer(timer), gamma(gamma) {}
...
void RadialEulerOperator<dim, degree, n_points_1d>::local_apply_cell(
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second;
        ...
        for (const unsigned int q : phi.quadrature_point_indices()) {
            ...
            phi.submit_gradient(radial_euler_flux<dim, VectorizedArray<Number>>(w_q, gamma), q);

            Tensor<1, dim+2, VectorizedArray<Number>> source;
            const auto rp_q = radial_euler_pressure<dim, VectorizedArray<Number>>(w_q, gamma);
            const auto r = phi.quadrature_point(q)[0];
            source[1] = rp_q / r;
            //std::cout << "p: " << source[1] << ", rpq: " << rp_q << std::endl;
...
void RadialEulerOperator<dim, degree, n_points_1d>::local_apply_boundary_face(
    ...
    for (unsigned int face = face_range.first; face < face_range.second;
        ...
        for (const unsigned int q : phi.quadrature_point_indices()) {
            ...
            if (inflow_boundaries.find(boundary_id) != inflow_boundaries.end()) {
            ...
            else if (axial_boundaries.find(boundary_id) !=
                    axial_boundaries.end()) {
                // Copy out density
                w_p[0] = w_m[0];
            ...
            else if (wall_boundaries.find(boundary_id) !=
                    wall_boundaries.end()) {
                w_p[0] = w_m[0];
            ...
            auto flux =
                radial_euler_numerical_flux<dim>(w_m, w_p, normal, r, gamma, false);

            if (axial_boundaries.find(boundary_id) !=
                    axial_boundaries.end()) {
...
template <int dim, int degree, int n_points_1d>
void RadialEulerOperator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &cell_range) const {
...
template <int dim, int degree, int n_points_1d>
void RadialEulerOperator<dim, degree, n_points_1d>::set_inflow_boundary(
    const types::boundary_id boundary_id,
    std::unique_ptr<Function<dim>> inflow_function) {
...
template <int dim, int degree, int n_points_1d>
void RadialEulerOperator<dim, degree, n_points_1d>::set_subsonic_outflow_boundary(
    const types::boundary_id boundary_id,
...
template <int dim, int degree, int n_points_1d>
void RadialEulerOperator<dim, degree, n_points_1d>::set_supersonic_outflow_boundary(
    const types::boundary_id boundary_id) {
...
template <int dim, int degree, int n_points_1d>
void RadialEulerOperator<dim, degree, n_points_1d>::set_axial_boundary(
    const types::boundary_id boundary_id) {
    AssertThrow(set_boundary_ids.find(boundary_id) == set_boundary_ids.end(),
                ExcMessage("You already set the boundary with id " +
                           std::to_string(static_cast<int>(boundary_id)) +
...
template <int dim, int degree, int n_points_1d>
void RadialEulerOperator<dim, degree, n_points_1d>::set_wall_boundary(
    const types::boundary_id boundary_id) {
    AssertThrow(set_boundary_ids.find(boundary_id) == set_boundary_ids.end(),
                ExcMessage("You already set the boundary with id " +
                           std::to_string(static_cast<int>(boundary_id)) +
...
void RadialEulerOperator<dim, degree, n_points_1d>::project(
  ...
  template <int dim, int degree, int n_points_1d>
  double RadialEulerOperator<dim, degree, n_points_1d>::compute_cell_transport_speed(
    const LinearAlgebra::distributed::Vector<Number> &solution) const
  {
